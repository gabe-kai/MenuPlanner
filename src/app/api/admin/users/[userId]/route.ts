...
import { deleteAuthIdentity } from "@/lib/auth/authIdentity.server";
...
export async function DELETE(
  ...
  const deleted = await deleteAuthIdentity(targetUserId);
  if (!deleted) {
  ...
  log.info({
    ...
    data: { actorUserId: actor.userId, targetUserId, familyId: actor.familyId },
  });
